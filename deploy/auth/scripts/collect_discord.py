...
使い方:
  ...
  # 本線（ルールv2・自動判定モード。新旧サーバー共通）: 全履歴バックフィル
  python collect_discord.py --channel <ID> --author 891983769358196756 \
      --rule-v2 --clean --out drafts

  # 日次運用: 直近2日分
  python collect_discord.py --channel <ID> --author 891983769358196756 \
      --rule-v2 --clean --days 2 --out drafts

  # 全履歴を遡って収集（1500回分バックフィル用・旧アンカーモード）
...
# 日付ヘッダー判定（--rule-v2 ルール2）: 「3/16（木）06：30頃」等。
# 全角/半角の括弧・コロン両対応、末尾の「頃」は省略可。
DATE_HEADER_RE = re.compile(
    r"\d{1,2}/\d{1,2}\s*[（(][月火水木金土日][）)]\s*\d{1,2}[:：]\d{2}\s*頃?"
)
...
# --rule-v2 のクラスタ内メッセージ数上限（安全弁）
RULE_V2_MAX_CLUSTER_MESSAGES = 15
...
# --- 収集ルールv2（自動判定モード・--rule-v2） --------------------------------


def has_date_header(content):
    """DATE_HEADER_RE（例: 「3/16（木）06：30頃」）を含むか。"""
    return DATE_HEADER_RE.search(content or "") is not None


def split_into_clusters(messages):
    """時系列昇順のメッセージ列を「隣接間隔30分以内で繋がるかたまり」に分割する。

    他authorはあらかじめ除外済みの前提（呼び出し側でフィルタする）。
    戻り値: [[msg, ...], [msg, ...], ...]（時系列順のクラスタのリスト）
    """
    if not messages:
        return []
    clusters = [[messages[0]]]
    last_time = parse_created_at(messages[0])
    for msg in messages[1:]:
        cur_time = parse_created_at(msg)
        if (cur_time - last_time).total_seconds() > MAX_GAP_SECONDS:
            clusters.append([msg])
        else:
...
def group_messages_rule_v2(messages, author_id, min_chars=0, warnings=None):
    """収集ルールv2（自動判定モード）。日毎（JST）に対象authorの投稿だけを見て、
    ルール1（分析N回目）→ルール2（日付ヘッダー）→ルール3（朝4-10時窓）の優先順で
    1日1記事を組み立てる。

    - 他authorの投稿は最初から除外し、対象author投稿だけを時系列で扱う
      （割り込みで打ち切られない）
    - ルール1/2: 対象author投稿を30分以内クラスタに分割し、アンカー（ルール1は
      ANCHOR_RE該当、ルール2はDATE_HEADER_RE該当）を含むクラスタ全体を採用する。
      同日に複数のアンカー/クラスタが該当する場合は時系列順に結合して1記事にする。
    - ルール3: アンカーが無い日は朝4:00〜10:00 JSTの投稿を全て1記事にする
      （窓内に投稿が無ければその日は記事なし）
    - クラスタの上限メッセージ数は RULE_V2_MAX_CLUSTER_MESSAGES（超過分は切り捨て、警告を記録）
    - warnings にはメッセージ数超過の警告文字列を追記する（呼び出し側で確認リストに反映する）

    戻り値: [{"number": int|None, "messages": [msg, ...], "rule": 1|2|3}, ...]（JST日付昇順）
    """
    if warnings is None:
        warnings = []

    # 対象authorの投稿のみをJST日付ごとにグルーピング（他authorは最初から除外）
    by_date = {}
    order = []
    for msg in messages:
        if str(msg.get("author", {}).get("id")) != str(author_id):
        ...
        if min_chars and len(content) < min_chars:
            continue
        jst_date = parse_created_at(msg).astimezone(JST).date()
        if jst_date not in by_date:
            by_date[jst_date] = []
            order.append(jst_date)
        by_date[jst_date].append(msg)
    ...
    groups = []
    for jst_date in order:
        day_messages = by_date[jst_date]  # 時系列昇順（呼び出し元のmessagesが昇順のため維持される）

        has_rule1 = any(find_anchor_number(m.get("content", "")) is not None for m in day_messages)
        has_rule2 = (not has_rule1) and any(has_date_header(m.get("content", "")) for m in day_messages)

        if has_rule1 or has_rule2:
            rule = 1 if has_rule1 else 2
            is_anchor = (
                (lambda c: find_anchor_number(c) is not None)
                if rule == 1
                else has_date_header
            )

            clusters = split_into_clusters(day_messages)
            selected_messages = []
            for cluster in clusters:
                if any(is_anchor(m.get("content", "")) for m in cluster):
                    selected_messages.extend(cluster)

            if not selected_messages:
                # 理論上は起こらない想定（has_rule1/2がTrueならどれかのクラスタに含まれる）だが念のため
                continue

            if len(selected_messages) > RULE_V2_MAX_CLUSTER_MESSAGES:
                warnings.append(
                    f"{jst_date.isoformat()}: ルール{rule}のクラスタが"
                    f"{RULE_V2_MAX_CLUSTER_MESSAGES}件を超えました"
                    f"（{len(selected_messages)}件 → 先頭{RULE_V2_MAX_CLUSTER_MESSAGES}件に打ち切り）。"
                    "内容を確認してください。"
                )
                selected_messages = selected_messages[:RULE_V2_MAX_CLUSTER_MESSAGES]

            combined_content = "\n".join(m.get("content", "") or "" for m in selected_messages)
            number = find_anchor_number(combined_content)
            groups.append({"number": number, "messages": selected_messages, "rule": rule})
        else:
            # ルール3: 朝4:00〜10:00 JSTの投稿を全て1記事に（窓内に投稿が無ければ記事なし）
            window_messages = [
                m for m in day_messages if 4 <= parse_created_at(m).astimezone(JST).hour < 10
            ]
            if not window_messages:
                continue
            if len(window_messages) > RULE_V2_MAX_CLUSTER_MESSAGES:
                warnings.append(
                    f"{jst_date.isoformat()}: ルール3（時間窓）の投稿が"
                    f"{RULE_V2_MAX_CLUSTER_MESSAGES}件を超えました"
                    f"（{len(window_messages)}件 → 先頭{RULE_V2_MAX_CLUSTER_MESSAGES}件に打ち切り）。"
                    "内容を確認してください。"
                )
                window_messages = window_messages[:RULE_V2_MAX_CLUSTER_MESSAGES]
            combined_content = "\n".join(m.get("content", "") or "" for m in window_messages)
            number = find_anchor_number(combined_content)
            groups.append({"number": number, "messages": window_messages, "rule": 3})
...
def build_confirmation_report(entries, extra_warnings=None):
    """全記事の一覧＋警告を確認リスト.mdの本文として組み立てる。

    extra_warnings: 呼び出し側（--rule-v2のクラスタ上限打ち切りなど）で収集した
    追加の警告文字列のリスト。指定時は警告一覧の末尾に追加する。
    """
    lines = ["# 確認リスト", ""]
    lines.append("| 番号 | 掲載日 | タイトル | メッセージ数 | 画像数 | ルール |")
    lines.append("|---|---|---|---|---|---|")

    sorted_entries = sorted(entries, key=lambda e: (e["published_date"], e["number"] or 0))
    for e in sorted_entries:
        rule = e.get("rule")
        rule_label = f"ルール{rule}" if rule else "-"
        lines.append(
            f"| {e['number'] if e['number'] is not None else '-'} | {e['published_date'].isoformat()} | {e['title']} | "
            f"{e['message_count']} | {e['image_count']} | {rule_label} |"
        )
...
def collect(
    ...
    daily_window=None, min_chars=0, clean=False, rule_v2=False,
):
    ...
    for channel_id in channels:
        ...
        if rule_v2:
            groups = group_messages_rule_v2(
                messages, author_id, min_chars=min_chars, warnings=rule_v2_warnings
            )
            print(f"  {len(groups)} 日分の投稿をまとめました（ルールv2・自動判定モード）")
        elif daily_window is not None:
    ...
    report = build_confirmation_report(entries, extra_warnings=rule_v2_warnings)
...
def main():
    ...
    parser.add_argument(
        "--rule-v2", action="store_true",
        help="収集ルールv2（自動判定モード）。新旧サーバー共通・過去/日次共通の本線モード。"
             "日毎（JST）に対象author投稿だけを見て「分析N回目」→日付ヘッダー→朝4-10時窓の"
             "優先順で1日1記事を組み立てる（--anchor-keyword・--daily-windowとは併用不可）"
    )
    ...
    if args.rule_v2 and (args.daily_window is not None or args.anchor_keyword is not None):
        print("--rule-v2 は --anchor-keyword / --daily-window と同時に指定できません。", file=sys.stderr)
        sys.exit(1)
